...
usage() {
  cat <<EOF
ktool v6 - Mini Deploy Tool (Kind)
...
cmd_build_image() {
  ...
  echo "$APP_IMAGE_TAG" > .last_image_tag

  if [[ -z "$APP_IMAGE_TAG" && -f .last_image_tag ]]; then
    APP_IMAGE_TAG=$(cat .last_image_tag)
  fi  
  ...
  docker build -t "${APP_IMAGE}" --label "build.timestamp=${APP_IMAGE_TAG}" . || { error "Falha ao construir a imagem Docker."; exit 1; }
...
apply_app() {
  ...
  local versioned_deploy_name="${base_app_name}-${version_suffix}"
  ...
  # 2. Substitui o nome do Deployment para incluir a versão
  # 3. Garante que os seletores de labels continuem os mesmos para o Service não quebrar
  cp "$APP_MANIFEST" "$temp_manifest"
  
  # Ajusta o nome do Deployment e a Imagem (apenas matches exatos de metadata name)
  sed -i "s/  name: ${base_app_name}$/  name: ${versioned_deploy_name}/" "$temp_manifest"
  sed -i "s|image: ${APP_IMAGE_REPO}.*|image: ${APP_IMAGE}|" "$temp_manifest"
  sed -i "s|app: ${base_app_name}|app: ${base_app_name}|" "$temp_manifest" # Preserva o label original
...
diagnose_failure() {
  ...
  local pod
  # Busca o pod pelo label original, mas que pertença ao deployment atual
  pod=$(kubectl get pods -n "$NAMESPACE" \
    --field-selector=status.phase!=Terminating \
    -o jsonpath="{.items[?(@.metadata.ownerReferences[0].name=='$APP_NAME')].metadata.name}" | awk '{print $1}')
  ...
  if kubectl get pods -n "$NAMESPACE" | grep "$APP_NAME" | grep -q CrashLoopBackOff; then
    separator
...
wait_rollout() {
  ...
  # Espera pelos pods que pertencem ao Deployment versionado especifico
  run kubectl wait --for=condition=ready pod \
    -n "$NAMESPACE" \
    --timeout="$TIMEOUT" \
    --selector=app=wallet-api
  verify_profile
}
...
cmd_status() {
  ...
  run kubectl get deploy -n "$NAMESPACE" -l app=wallet-api
  run kubectl get pods -n "$NAMESPACE"
...
cmd_logs() {
  ...
  if kubectl get deployment "$APP_NAME" -n "$NAMESPACE" >/dev/null 2>&1; then
    target_deploy="$APP_NAME"
  else
    info "Deployment não encontrado. Buscando mais recente..."
    target_deploy=$(resolve_latest_deployment)
  fi
  ...
  if [[ "$FOLLOW" == "true" ]]; then
    run kubectl logs -f "$pod" -n "$NAMESPACE" --tail="$TAIL"
  else
    run kubectl logs "$pod" -n "$NAMESPACE" --tail="$TAIL"
  fi
}


resolve_latest_deployment() {
  # Busca deployments com label app=wallet-api e pega o mais recente
  local latest_deploy

  latest_deploy=$(kubectl get deployments -n "$NAMESPACE" \
    -l app=wallet-api \
    --sort-by=.metadata.creationTimestamp \
    -o jsonpath='{.items[-1:].metadata.name}' 2>/dev/null || true)

  if [[ -z "$latest_deploy" ]]; then
    error "Nenhum deployment encontrado com label app=wallet-api"
    exit 1
  fi
